...
import { ServerGroupReport, doShit as doStuff } from './controls';
...
function loginToServer(serverName: string) {
	...
	let newtab: JQuery<WebviewTag> = $(`<webview id="${server}" src="http://${server}/evo2/fresh/login.asp" webpreferences="disableDialogs" preload="./dist/embedded.js"></webview>`);
 ...
 */
function prepareReports(serverGroups: ServerGroup[])
{
	$(".group-report").remove();
...
function init()
{
	...
	$(".group-login-button").on("click", function() {
		loginToGroup($(this).attr("id") as string);
	});
...
function loginToGroup(buttonId: string) {
	if (hasLoginCredentials())
	{
		let groupId = buttonId.substr(0, buttonId.indexOf("-button"));
		let reportRow = $(`#${groupId}-report tr.waiting, #${groupId}-report tr.bad`) as JQuery<HTMLTableRowElement>;
